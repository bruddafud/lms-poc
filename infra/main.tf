locals {
  ...
  sha                          = base64encode(sha256("earth${var.environment_name}${var.location}${data.azurerm_client_config.current.subscription_id}"))
...
module "keyvault" {
  ...
  access_policy_object_ids = [
    module.api.IDENTITY_PRINCIPAL_ID
    ]
  secrets = [
    {
...
module "appserviceplan" {
  ...
  # free plan, very limited, high latency, see web section
  sku_name = "F1"
}
